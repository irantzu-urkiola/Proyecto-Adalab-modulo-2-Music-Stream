...
CREATE TABLE popularidad_flamencopop AS
...
WITH Top20Songs AS (
    -- 1. Identificar las 20 canciones más populares de Flamenco Pop
...
Top20Artistas AS (
    ...
    FROM Top20Songs t20
    JOIN canciones_artistas_flamencopop ca ON t20.id_cancion = ca.id_cancion
    JOIN flamencopop_artistas a ON ca.id_artista = a.id_artista
),
...
FROM flamencopop f
INNER JOIN flamencopop_artistas a
...
SELECT * FROM flamencopop; -- Manu Carrasco, Estopa y Antonio Orozco

SELECT cancion, album, nombre_artista, popularidad
FROM flamencopop
WHERE UPPER(nombre_artista) = 'MANUEL CARRASCO'
ORDER BY popularidad DESC
LIMIT 20;
SELECT cancion, album, nombre_artista, popularidad
FROM flamencopop
WHERE UPPER(nombre_artista) = 'ESTOPA'
ORDER BY popularidad DESC
LIMIT 20;
SELECT cancion, album, nombre_artista, popularidad
FROM flamencopop
WHERE UPPER(nombre_artista) = 'ANTONIO OROZCO'
ORDER BY popularidad DESC
LIMIT 20;
